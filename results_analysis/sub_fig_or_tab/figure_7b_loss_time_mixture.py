...
def func_2_5(first_tp, first_fp, second_tp, second_fp, second_fn):
    return loss_time_vm_migration * (first_tp + first_fp) + loss_time_vm_migration * (
            second_tp + second_fp) + 5 * 60 * (second_fn)


def func_2_50(first_tp, first_fp, second_tp, second_fp, second_fn):
    return loss_time_vm_migration * (first_tp + first_fp) + loss_time_vm_migration * (
            second_tp + second_fp) + 50 * 60 * (second_fn)


def func_2_500(first_tp, first_fp, second_tp, second_fp, second_fn):
    return loss_time_vm_migration * (first_tp + first_fp) + loss_time_vm_migration * (
...
def start_plot(seed_value):
    for i in [0.4]:
        ...
        for j in [0.84]:
            ...
            mix_5 = func_2_5(first_tp, first_fp, second_tp, second_fp, second_fn)
            mix_50 = func_2_50(first_tp, first_fp, second_tp, second_fp, second_fn)
            mix_500 = func_2_500(first_tp, first_fp, second_tp, second_fp, second_fn)
            return [mix_5, mix_50, mix_500]


def save_txt(mix_loss_time):
    with open(base_path + 'temp_dir/figure_7b_mixture.csv', 'w') as file:
        file.write(','.join(map(str, mix_loss_time)) + '\n')
